...
+ (void)updateToVersion:(NSInteger)version callback:(JPUpdateCallback)callback
{
    ...
    // create url request
    NSString *downloadKey = [NSString stringWithFormat:@"/%@_v%@.zip?v=%f", appVersion, @(version), [[NSDate date] timeIntervalSince1970]];
    ...
    NSURLSessionDataTask *task = [[NSURLSession sharedSession] dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        ...
        if (!error) {
            ...
            if (!isFailed) {
                NSData *md5Data = [RSAUtil decryptData:[NSData dataWithContentsOfFile:keyFilePath] publicKey:publicKey];
